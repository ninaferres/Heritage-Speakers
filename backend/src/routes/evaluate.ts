...
const writingSchema = z.object({ level: cefrLevel, prompt: z.string(), text: z.string().min(1) });
...
evaluateRouter.post('/writing', async (req, res) => {
  ...
  try {
    const result = await gradeWriting(parsed.data.level, parsed.data.prompt, parsed.data.text);
    res.json(result);
  } catch (err) {
...
evaluateRouter.post('/reading', async (req, res) => {
  ...
  try {
    const result = await gradeReading(parsed.data.level, parsed.data.passage, parsed.data.questions, parsed.data.answers);
    res.json(result);
  } catch (err) {
    handleGradingError(res, err);
  }
...
evaluateRouter.post('/listening', async (req, res) => {
  ...
  try {
    const result = await gradeListening(parsed.data.level, parsed.data.transcript, parsed.data.questions, parsed.data.answers);
    res.json(result);
  } catch (err) {
    handleGradingError(res, err);
  }
...
evaluateRouter.post('/speaking', upload.single('audio'), async (req, res) => {
  ...
  try {
    const result = await gradeSpeaking(level.data, prompt, req.file.buffer, req.file.originalname, req.file.mimetype);
    res.json(result);
  } catch (err) {
    handleGradingError(res, err);
  }
